...
#scatterplotting
shapes <- c("Greenling" = 20, "China" = 2, "Quillback" = 1, "Chlor_a" = 8)

ggplot(dat, aes(x=Year)) +
  geom_point(aes(y=Greenling, shape="Greenling"), size=5) + geom_smooth(aes(y=Greenling), color = "grey", se=FALSE) +
  geom_point(aes(y=Canary, shape="China"), size=5) + geom_smooth(aes(y=Canary), color = "grey", se=FALSE) +
...
ggplot(dat, aes(x=Year)) +
  ...
  geom_point(aes(y=China), size=4, color="blue") + 
...
p1 <- ggplot(dat, aes(x=Year)) +
  ...
  annotate("text", x=2010, y=1.5, label="Tau= -0.53") +
  annotate("text", x=2010, y=1, label="p=0.026*") +
  ggtitle("A. Greenling") +
...
p2 <- ggplot(dat, aes(x=Year)) +
  geom_point(aes(y=China), size=3) + 
  theme_cowplot() + labs(y="Mean count", x="Year") +
  geom_smooth(aes(y=Chlor_a), se=FALSE) +
  scale_y_continuous(trans="log2", sec.axis=dup_axis(name="Chlor a (mg/L)")) +
  annotate("text", x=2008, y=1.5, label="Tau= -0.44") +
  annotate("text", x=2008, y=1, label="p=0.061") +
  ggtitle("B. China") +
  theme(axis.title.y.right=element_text(color="blue"), 
        axis.text.y.right=element_text(color="blue"),
        axis.title.x=element_blank(),
...
p3 <- ggplot(dat, aes(x=Year)) +
  ...
  geom_smooth(aes(y=Chlor_a), se=FALSE) +
  scale_y_continuous(trans="log2", sec.axis=dup_axis(name="Chlor a (mg/L)")) +
  annotate("text", x=2009, y=1, label="Tau= -0.56") +
  annotate("text", x=2009, y=0.7, label="p=0.02*") +
  ggtitle("C. Quillback") +
  theme(axis.title.y.right=element_text(color="blue"), 
        axis.text.y.right=element_text(color="blue"),
        axis.title.x=element_blank(),
...
#Correlations
cor.DO <- cor.test(dat$Puget_Sound, dat$DO, method = "kendall")
print(cor.DO)
#p<0.05 for Vermillion

cor.temp <- cor.test(dat$Puget_Sound, dat$Temperature, method = "kendall")
print(cor.temp)
#p<0.1 for Yelloweye and Greenling

cor.chl <- cor.test(dat$Greenling, dat$Chlor_a, method = "kendall")
print(cor.chl)
